...
def create_bg1_mask(image):
    ...
    image_grey = median(image_grey) 
    for i in range(1,5):
        image_denoised = denoise_bilateral(image_grey, sigma_spatial=2, multichannel=False)
...
#        image_cropped = denoise_bilateral(image_cropped, sigma_spatial=2, multichannel=False)
    image_cropped[image_cropped >= 0.9] = 1
    ...
    for blob in blobs_doh:
        ...
        ax.add_patch(c)
        p = True
        if (r >1) and (r <20) and (x > 30) and (x < y_tot-30) and (y > 50) and (y < x_tot -50):
