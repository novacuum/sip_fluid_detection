...
switch layerName
    ...
    case {'roughILMandISOS'}
        ...
        disp('getHyperReflectiveLayers done')
        %save to structure 
        clear rPaths
        rPaths = pathsTemp;
...
switch layerName
    ...
    case {'rpe' 'nflgcl' 'oplonl' 'iplinl' }
        disp('layersCore search before')
        adjMatrixW = sparse(adjMA(keepInd),adjMB(keepInd),adjMW(keepInd),numel(img(:)),numel(img(:)));    
        [ ~, path ] = dijkstra( adjMatrixW, 1, numel(img(:)) );
        disp('layersCore search after')
        % dist = nan(size(path));
    ...
    case {'inlopl' 'ilm' 'isos' }
        ...
        [ ~, path ] = dijkstra( adjMatrixMW, 1, numel(img(:)) );
        disp('layersCore search after')
        % dist = nan(size(path));
